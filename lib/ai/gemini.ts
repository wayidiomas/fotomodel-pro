 ...
 *
 * Capabilities:
 * - Text-to-Image generation (1290 tokens per image, ~3.2s latency)
 ...
 *
 * Pricing: $30/1M output tokens = $0.039 per image
...
/**
 * Generate an image from text prompt using Gemini 2.5 Flash Image
 *
...
export async function generateImage(
  ...
  try {
    ...
    const modelId = process.env.GEMINI_MODEL_ID || 'gemini-2.5-flash-image';
 ...
 *
 * Supports up to 3 input images for composition tasks
 *
 * @param options - Blend options including prompt and up to 3 images
...
export async function blendImages(
  ...
  try {
    ...
    // Validate image count (max 3 images supported)
    if (options.images.length > 3) {
      return {
        success: false,
        error: 'Maximum 3 images supported for blending',
      };
    }
 ...
 * Uses multi-image composition (2 images) with Gemini 2.5 Flash Image.
 *
 * Best practices:
